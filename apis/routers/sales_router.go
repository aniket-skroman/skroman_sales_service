...
func SalesRouter(router *gin.Engine, store *apis.Store) {
	...
	sales_service = services.NewSalesLeadService(sales_repository, lead_order_ser, jwt_servive)
	sales_controller = controller.NewSaleLeadsController(sales_service)

	sales_lead := router.Group("/api")
	{
		...
		sales_lead.GET("/sales-leads-status/:page_id/:page_size/:status", sales_controller.FetchLeadsByStatus)
		sales_lead.GET("/sale_lead_counts", sales_controller.FetchLeadCounts)
		sales_lead.PUT("/sales-lead", sales_controller.CancelLead)
		sales_lead.GET("/cancel-lead/:lead_id", sales_controller.FetchCancelLead)
	}
